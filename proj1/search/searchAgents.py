...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if actualState in self.corners:
            if actualState not in visitedCorner:
                visitedCorner.append(actualState)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                nextVisitedCorner = list(visitedCorner)
                if nextState in self.corners and nextState not in nextVisitedCorner:
...
def cornersHeuristic(state, problem):
    ...
    visitedCorner = state[1]
    unvisitedCorner = [c for c in corners if c not in visitedCorner]
    heuristics = []

    while len(unvisitedCorner) > 0:
        distance, corner = min([(util.manhattanDistance(currentState, c), c) for c in unvisitedCorner])
        heuristics += [distance]
        currentState = corner
        unvisitedCorner.remove(corner)
